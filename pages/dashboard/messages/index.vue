  ...
  <MessagingView>
    <div>
      ...
      <div class="relative -mt-32 h-screen flex lg:flex-row flex-col">
        ...
        <div class="w-full lg:w-[500px] border-r border-gray-200 flex flex-col bg-white overflow-y-auto" v-show="openSideNav || isDesktop">
          <div class="flex items-center justify-between p-4 bg-gray-100">
            <div class="relative flex items-center bg-gray-200 rounded-lg px-3 py-2 w-full">
              ...
              <input v-model="searchQuery" type="text" class="bg-gray-200 text-gray-600 text-sm ml-2 py-1.5 focus:outline-none w-full" placeholder="Search" />
            </div>
            <button class="lg:hidden p-2" @click="openSideNav = false">✕</button>
          </div>

          <div v-if="!loadingActiveChats && activeChatsList.length" class="overflow-auto flex-1">
            <ChatUserList class="p-3" :loading="loadingActiveChats" :users="activeChatsList" :filteredUsers="filteredUserList" @selectUser="selectUser" />
          </div>
        ...
        <div class="flex-1 flex flex-col bg-white h-full" v-show="selectedUser || isDesktop">
          <div class="flex items-center justify-between p-4 bg-gray-100 border-b border-gray-200">
            <button class="lg:hidden p-2" @click="openSideNav = true">☰</button>
            <ChatHeader :selectedUser="selectedUser || roomChatsList" :isConnected="isConnected" />
          </div>
          ...
          <ChatMessageInput v-model="newMessage" :isConnected="isConnected" :isSending="messageStatus === 'sending'" @sendMessage="sendMessageToUser" />
        </div>
      </div>
    </div>
  </MessagingView>
...
<path d="M12.107 22.6218C11.9298 23.7835 12.722 24.5898 13.6921 24.9916C17.411 26.5323 22.5864 26.5323 26.3053 24.9916C27.2754 24.5898 28.0676 23.7835 27.8905 22.6218C27.7815 21.9079 27.243 21.3134 26.844 20.7329C26.3214 19.9632 26.2695 19.1236 26.2695 18.2305C26.2695 14.7787 23.4619 11.9805 19.9987 11.9805C16.5355 11.9805 13.728 14.7787 13.728 18.2305C13.7279 19.1236 13.676 19.9632 13.1534 20.7329C12.7544 21.3134 12.2159 21.9079 12.107 22.6218Z" stroke="#1D2739" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M16.668 26.1445C17.05 27.5822 18.3976 28.6445 20.0013 28.6445C21.6051 28.6445 22.9526 27.5822 23.3346 26.1445" stroke="#1D2739" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
<svg width="40" height="41" viewBox="0 0 40 41" fill="none" xmlns="http://www.w3.org/2000/svg">
<rect y="0.3125" width="40" height="40" rx="8" fill="#292929"/>
<path d="M20.0817 20.4943C20.0817 21.7367 19.8544 22.8049 19.3999 23.6989C18.9453 24.589 18.3222 25.2746 17.5305 25.7557C16.7427 26.233 15.8468 26.4716 14.843 26.4716C13.8355 26.4716 12.9358 26.233 12.1442 25.7557C11.3563 25.2746 10.7351 24.5871 10.2805 23.6932C9.82599 22.7992 9.59872 21.733 9.59872 20.4943C9.59872 19.2519 9.82599 18.1856 10.2805 17.2955C10.7351 16.4015 11.3563 15.7159 12.1442 15.2386C12.9358 14.7576 13.8355 14.517 14.843 14.517C15.8468 14.517 16.7427 14.7576 17.5305 15.2386C18.3222 15.7159 18.9453 16.4015 19.3999 17.2955C19.8544 18.1856 20.0817 19.2519 20.0817 20.4943ZM18.343 20.4943C18.343 19.5473 18.1896 18.75 17.8828 18.1023C17.5798 17.4508 17.1631 16.9583 16.6328 16.625C16.1063 16.2879 15.5097 16.1193 14.843 16.1193C14.1726 16.1193 13.5741 16.2879 13.0476 16.625C12.5211 16.9583 12.1044 17.4508 11.7976 18.1023C11.4946 18.75 11.343 19.5473 11.343 20.4943C11.343 21.4413 11.4946 22.2405 11.7976 22.892C12.1044 23.5398 12.5211 24.0322 13.0476 24.3693C13.5741 24.7027 14.1726 24.8693 14.843 24.8693C15.5097 24.8693 16.1063 24.7027 16.6328 24.3693C17.1631 24.0322 17.5798 23.5398 17.8828 22.892C18.1896 22.2405 18.343 21.4413 18.343 20.4943ZM22.2706 26.3125V14.6761H26.4183C27.3198 14.6761 28.0679 14.8314 28.6626 15.142C29.2611 15.4527 29.7081 15.8826 30.0036 16.4318C30.299 16.9773 30.4467 17.608 30.4467 18.3239C30.4467 19.036 30.2971 19.6629 29.9979 20.2045C29.7024 20.7424 29.2554 21.161 28.657 21.4602C28.0623 21.7595 27.3142 21.9091 26.4126 21.9091H23.2706V20.3977H26.2536C26.8217 20.3977 27.2839 20.3163 27.6399 20.1534C27.9998 19.9905 28.263 19.7538 28.4297 19.4432C28.5964 19.1326 28.6797 18.7595 28.6797 18.3239C28.6797 17.8845 28.5945 17.5038 28.424 17.1818C28.2573 16.8598 27.9941 16.6136 27.6342 16.4432C27.2782 16.2689 26.8104 16.1818 26.2308 16.1818H24.0263V26.3125H22.2706ZM28.0149 21.0625L30.8899 26.3125H28.8899L26.0717 21.0625H28.0149Z" fill="#EBE5E0"/>
</svg>
