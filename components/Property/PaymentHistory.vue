<template>
  <main class="">
    ...
    <div class="border-[0.5px] border-gray-100 rounded-lg">
      <div class="bg-white border-[0.5px] border-gray-50 rounded-lg overflow-hidden">
        <div class="custom-scrollbar-container w-full">
          <table class="min-w-full bg-white w-full">
            ...
            <tbody v-if="!loadingPaymentHistory" class="divide-y divide-gray-50 z-10">
              <tr class="cursor-pointer" v-for="(item, index) in paymentList" :key="item.id">
                ...
                <td class="py-5 px-5 whitespace-nowrap text-left text-sm text-[#667185] font-semibold relative">
                  <p>
                    {{ item?.paymentsMade ?? 'Nil' }}
                  </p>
                </td>
                <!-- <td class="py-5 px-5 whitespace-nowrap text-left text-sm text-[#667185] font-semibold relative">
                  <p>
                    <span class="rounded-full px-3 py-2 text-sm font-semibold"
                      :class="[item.tenant.status === 'not paid' ? 'text-[#BA110B] bg-[#FBEAE9]' : 'text-[#039855] bg-[#E7F6EC]']">
                ...
                <td class="py-5 px-5 whitespace-nowrap text-left text-sm text-[#667185] font-semibold relative">
                  <p>
                    <span class="rounded-full px-3 py-2 text-sm font-semibold"
                      :class="[item.currentRentStatus=== 'not paid' ? 'text-[#BA110B] bg-[#FBEAE9]' : 'text-[#039855] bg-[#E7F6EC]']">
                      {{ item?.currentRentStatus.toUpperCase() ?? 'Nil' }}
                    </span>
                  </p>
                </td>
                <td class="py-5 px-5 whitespace-nowrap text-left text-sm text-[#667185] font-semibold relative">
        ...
        <CorePagination v-if="!loadingPaymentHistory && paymentList.length > 0" :total="metadata.total"
          :page="metadata.page" :perPage="metadata.perPage" :pages="metadata.pages" @page-changed="handlePageChange" />
      </div>
      <div v-if="!loadingPaymentHistory && paymentList.length === 0"
