<template>
  ...
  <li
    ...
    :key="user.id"
    :class="{
    'bg-gray-100': user?.participant?.id &&
      user?.participant?.id === selectedUserChat?.participant?.id
  }"
  ...
  <!-- {{user}} -->
   <!-- {{selectedUserChat}} -->
    <div class="flex items-center w-full">
      ...
      <img
        v-if="user.type === 'activeChat' && user.participant.profilePicture"
        :src="user.participant.profilePicture"
        class="w-10 h-10 rounded-full mr-3"
        alt=""
      />
      <!-- <img
        v-if="user.type === 'member' && user.profilePicture"
      ...
      <div class="w-full space-y-1 pl-3">
        <div class="flex justify-between items-center w-full">
          <p>
            {{ user.type === 'activeChat' ? `${user.participant.firstName} ${user.participant.lastName}` : `${user.firstName} ${user.lastName}` }}
          </p>
        ...
        <!-- <li
          ...
          :class="{'bg-gray-100' : user?.participant?.id === selectedUserChat?.participant?.id}"
          @click="selectUser(user)"
  ...
  (newVal) => {
    if (Array.isArray(newVal) && newVal.length >= 1) {
      selectUser(newVal[0]); // Call selectUser with the first user in the array
      selectedUserChat.value = newVal[0]; // Update the selected user chat
      console.log('One or more items found:', newVal);
    }
  },
  { immediate: true }
);
...
//   if (Array.isArray(newVal) && newVal.length === 1) {
//     selectUser(newVal[0]); // Call selectUser with the only user
//     selectedUserChat.value = newVal[0]; // Update the selected user chat
//     console.log('Only one item found:', newVal[0]);
//   }
// }, { immediate: true });
...
//   if(newVal.length === 1) {
//     selectUser(newVal[0])
//     selectedUserChat.value = newVal[0]
