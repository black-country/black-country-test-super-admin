<template>
  <main>
    <div class="flex flex-wrap gap-5 justify-between items-center mb-4">
      <div class="flex space-x-2">
        <button @click="propertyFilterModal = true"
          class="flex items-center text-sm space-x-1 bg-[#F0F2F5] border-[#F0F2F5] border text-gray-700 px-2 py-2 rounded transition-all">
          <img :src="dynamicIcons('gray-filter')" />
          <span>Filter</span>
        </button>
        <div class="relative w-full">
          <input v-model="filters.searchQuery" type="text" placeholder="Search members by name..."
    ...
    <div>
      <div v-if="membersList && !loadingMembers" class="bg-white rounded-lg overflow-hidden">
        <div class="custom-scrollbar-container">
          <table class="min-w-full bg-white">
            ...
            <tbody class="divide-y divide-gray-50 z-10">
              <tr class="cursor-pointer" v-for="(member, index) in processedData" :key="member.id">
                <td @click.prevent="handleDropdownClick('view', member)" v-for="column in visibleColumns"
                  ...
                  <p class="absolute left-0 top-10" v-if="column.label === 'Status'">
                    <span :class="[member.isActive ? 'bg-green-500' : 'bg-red-500']"
                      class="rounded-full px-3 py-2 text-white text-sm">
                ...
                <td class="py-5 px-5 whitespace-nowrap text-sm text-right">
                  ...
                  <div v-if="activeDropdown === index"
                    ...
                    <ul class="py-1 text-sm text-gray-700 divide divide-y-[0.5px]">
                      <li>
                        <a @click.prevent="handleDropdownClick('view', member)" href="#"
                          class="block flex items-center gap-x-2 px-4 py-3 hover:bg-gray-100 text-start">
                      ...
                      <li>
                        <a @click.prevent="handleDropdownClick('deactivate', member)" href="#"
                          class="block flex items-center gap-x-2 px-4 py-3 hover:bg-gray-100 text-start">
                      ...
                      <li>
                        <a @click.prevent="handleDropdownClick('remove', member)" href="#"
                          class="block flex items-center gap-x-2 px-4 py-3 hover:bg-gray-100 text-start">
        ...
        <CorePagination v-if="!loadingMembers && membersList.length > 0" :total="metadata.total" :page="metadata.page"
          :perPage="metadata.perPage" :pages="metadata.pages" @page-changed="handlePageChange" />
      </div>
      <div v-if="loadingMembers && membersList.length === 0" class="h-32 bg-slate-200 rounded animate-pulse w-full m-3">
      </div>
      <div v-if="!loadingMembers && membersList.length === 0" class="flex justify-center items-center flex-col my-20">
...
const downloadData = (exportType: any) => {
  const columnsToExport = visibleColumns.value.flatMap(column => column.exportKeys);
  const uniqueColumnsToExport = [...new Set(columnsToExport)];
  exportPaginatedData('/members', exportType, 'members_data_export', uniqueColumnsToExport, membersCustomHeaders);
};
...
const columns = ref([
  { label: "Full Name", key: "fullName", exportKeys: ["firstName", "lastName"], visible: true },
  { label: "Email", key: "email", exportKeys: ["email"], visible: true },
  { label: "Role", key: "group", exportKeys: ["role", "group"], visible: true },
  { label: "Status", key: "isActive", exportKeys: ["isActive"], visible: true },
  { label: "Last Active", key: "lastActive", exportKeys: ["lastActive"], visible: false },
  { label: "Group", key: "group", exportKeys: ["role", "group"], visible: false },
  { label: "Date Added", key: "createdAt", exportKeys: ["createdAt"], visible: false },
]);
