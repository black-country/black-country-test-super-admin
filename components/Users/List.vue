  ...
  <main class="">
    <div class="flex flex-wrap gap-3  justify-between items-center mb-4">
      ...
      <div class="flex space-x-2">
        <!-- Filter Button -->
        <button @click="tenantFilterModal = true"
      ...
      <div class="bg-white rounded-lg overflow-hidden">
        <div class="custom-scrollbar-container w-full">
          <table class="min-w-full bg-white w-full">
            ...
            <tbody v-if="!loadingUsers" class="divide-y divide-gray-50 z-10">
              <tr class="cursor-pointer" v-for="(tenant, index) in usersList" :key="tenant.id">
                <td @click.prevent="handleDropdownClick('view', tenant)" v-for="column in visibleColumns"
                  ...
                  <p class="inline px-3 py-2 rounded-full text-sm"
                    ...
                    v-else-if="column.key === 'status'">{{ tenant.status === 'not paid' ? 'Not Paid' : 'Paid' }}</p>
                ...
                <td class="py-5 px-5 relative whitespace-nowrap text-sm text-right">
                  ...
                  <div v-if="activeDropdown === index"
                    ...
                    <ul class="py-1 text-sm text-gray-700 divide divide-y-[0.5px]">
                      <li>
                        <a @click.prevent="handleDropdownClick('view', tenant)" href="#"
                          class="block flex items-center gap-x-2 px-4 py-3 hover:bg-gray-100 text-start">
                      ...
                      <li>
                        <a @click.prevent="handleDropdownClick('send-message', tenant)" href="#"
          ...
          <section id="loader" class="w-full" v-if="loadingUsers">
        ...
        <CorePagination v-if="!loadingUsers && usersList.length > 0" :total="metadata.total" :page="metadata.page"
          :perPage="metadata.perPage" :pages="metadata.pages" @page-changed="handlePageChange" />
      </div>
      <!-- <div
          v-if="loadingTenants && usersList.length === 0"
          class="h-32 bg-slate-200 rounded animate-pulse w-full m-3"
        ></div> -->
      <div v-if="!loadingUsers && usersList.length === 0" class="flex justify-center items-center flex-col my-20">
        <div class="flex justify-center flex-col items-center gap-y-4 items-center">
          <img src="@/assets/icons/activities-empty-state.svg" />
          <p class="font-medium text-gray-400">No tenant found for search key <span
    ...
    <TenantConfigTableModal v-if="tenantConfigModal" @close="tenantConfigModal = false" />
    <TenantFilterModal v-if="tenantFilterModal" @close="tenantFilterModal = false" @applyFilters="handleApplyFilters" />
...
import { useGetUsers } from '@/composables/modules/users/fetch'
...
const handlePageChange = (val: any) => {
  ...
  getUsers(); // Explicitly call the method to fetch new data
};
...
const handleDropdownClick = (action: any, item: any) => {
  if (action === "view") {
    return router.push(`/dashboard/users-mgt/${item.id}`);
  }
...
const handleExport = (type: string) => {
  if (type === "csv") {
    exportData(usersList.value, "csv", "tenant-listings");
  }

  if (type === "pdf") {
    exportData(usersList.value, "pdf", "tenant-listings");
  }

  if (type === "excel") {
    exportData(usersList.value, "excel", "tenant-listings");
  }
