<template>
  <Layout>
    <template #header-content>
      <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6 justify-between">
        <div class="flex items-center gap-x-4">
          <h4 class="text-[#1D2739] text-lg font-semibold">User Management</h4>
    ...
    <TenantsAllMaintenenceRequests :showTabs="true"  v-if="activeTab === 'maintenance-request'" />
  </Layout>
</template>
  ...
  watch(
      ...
      (newActiveTab) => {
        if (!newActiveTab?.length) {
          setActiveTab('users'); // Set default active tab
        } else {
