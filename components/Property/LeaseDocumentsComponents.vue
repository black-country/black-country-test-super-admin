      ...
      <div>
        <section class="lg:flex justify-between items-center overflow-x-auto">
          <div class="flex space-x-6 mb-4 overflow-x-auto scrollbar-hide">
            <span @click="activeTab = 'all'" :class="[
              activeTab === 'all'
            ...
            <span @click="activeTab = 'pending'" :class="[
              activeTab === 'pending'
                ? 'border-[#5B8469] border-b-2 text-[#5B8469]'
                : 'text-gray-700',
            ]" class="text-sm font-medium text-gray-500 pb-2 cursor-pointer whitespace-nowrap">
            ...
            <span @click="activeTab = 'signed'" :class="[
              activeTab === 'signed'
                ? 'border-[#5B8469] border-b-2 text-[#5B8469]'
                : 'text-gray-700',
            ]" class="text-sm font-medium text-gray-500 pb-2 cursor-pointer whitespace-nowrap">
            ...
            <span @click="activeTab = 'draft'" :class="[
              activeTab === 'draft'
                ? 'border-[#5B8469] border-b-2 text-[#5B8469]'
                : 'text-gray-700',
            ]" class="text-sm font-medium text-gray-500 pb-2 cursor-pointer whitespace-nowrap">
        ...
        <section v-if="filteredLeaseAgreements.length && !loading"
          class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div class="bg-white rounded-lg border-[0.5px] p-4" v-for="lease in filteredLeaseAgreements" :key="lease.id">
            <!-- {{lease}} -->
            <div class="flex justify-between items-start h-full">
              <div class="space-y-2 w-full">
                <div v-if="isLeaseAgreementContentEmpty(lease.leaseAgreementContent)"
                ...
                <section v-else @click="viewTemplate(lease)">
                  ...
                  <div v-if="!containsHttps(lease?.leaseAgreementContent)"
                    v-html="shortenedText(lease?.leaseAgreementContent)"
                    class="p-4 rounded-lg text-[10px] overflow-hidden min-h-[150px] max-h-[150px]"></div>
                  <div v-else>
                    <iframe
                ...
                <div class="flex justify-between items-center">
                  ...
                  <div class="relative whitespace-nowrap pb-5 pl-3 pr-4 text-right text-sm font-medium sm:pr-0">
                    <button class="p-2" @click="toggleDropdown(lease.id)" :id="'drop-options-' + lease.id">
                    ...
                    <div v-if="openDropdown === lease.id"
                      ...
                      <ul class="divide-y divide-gray-100">
                        ...
                        <li v-if="lease.status === 'DRAFT'" class="p-2 text-sm cursor-pointer">
                          <button @click="sendNow(lease)"
                            class="flex py-1.5 text-[#1D2739] items-center font-medium gap-x-3">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                              xmlns="http://www.w3.org/2000/svg">
                              <path
                                ...
                                stroke="#5B8469" stroke-width="1.5" />
                              <path d="M9.58398 10.4167L12.5007 7.5" stroke="#5B8469" stroke-width="1.5"
                        ...
                        <li v-if="lease.status === 'DRAFT'" class="p-2 text-sm cursor-pointer">
                          <button @click="duplicateTemplate(lease)"
                            class="flex py-1.5 text-[#1D2739] items-center font-medium gap-x-3">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                              xmlns="http://www.w3.org/2000/svg">
                              <path
                        ...
                        <li v-if="lease.status === 'SIGNED'" class="p-2 text-sm cursor-pointer">
                          <button :disabled="isDownloading" @click="downloadTemplate(lease)"
                        ...
                        <li v-if="lease.status === 'DRAFT'" class="p-2 text-sm cursor-pointer">
                          <button @click="deleteLeaseAgreement(lease)" :disabled="deleting"
                            class="flex py-1.5 text-[#1D2739] disabled:cursor-not-allowed disabled:opacity-25 items-center font-medium gap-x-3">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                              xmlns="http://www.w3.org/2000/svg">
                              <path
                              ...
                              <path d="M16.6673 12.5L10.834 18.3329M16.6673 18.3333L10.834 12.5004" stroke="#5B8469"
