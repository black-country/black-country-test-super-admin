<!-- <template>
  <div class="p-4 bg-white">
    <ul class="w-full scrollbar-visible max-h-[calc(100vh-2rem)] overflow-y-auto pb-10"
      v-if="users?.length && !loading">
      <ul>
        <li v-for="user in filteredUsers" :key="user.id" :class="{'bg-gray-100':
            user?.participant?.id &&
          ...
          <div class="flex items-center w-full">
            <img v-if="
              user.type === 'activeChat' && user.participant.profilePicture
            " :src="user.participant.profilePicture" class="w-10 h-10 rounded-full mr-3" alt="" />
...
<template>
  <div class="p-4 bg-white overflow-y-auto">
    <ul class="w-full scrollbar-visible max-h-[calc(100vh-2rem)] overflow-y-auto" v-if="users?.length && !loading">
...
/* Add some space at the bottom of the container */
.scrollbar-visible>ul:last-child {
  margin-bottom: 1rem;
}
</style>
